...
export const Chart: FC<Partial<EChartsCoreOption & ChartProps>> = (props) => {
  ...
  const  {
    ...
    onRendered, onFinished,
    width=600, height=300,
    autoResize, theme,
    notMerge, lazyUpdate, group,
    ...other
  } = props
  ...
  const removeOption = (key: string, id: string) => {
    if (options.current[key]) {
      // 1. 移除组件配置
      // @ts-ignore
      options.current = { ...options.current, [key]: options.current[key].filter((i: any) => i.id !== id)}
      // 2. 增加replaceMerge
      replaceMerge.current.add(key)
      // 3. 提交更新到echarts
      commit()
  ...
  return (
    <ChartContext.Provider value={{options, chart: chart.current, setOption, removeOption}}>
      <div ref={container} className={`echarts ${className}`} style={{
        display: 'block',
        width: '100%',
        height: '100%',
        ...(style as object),
